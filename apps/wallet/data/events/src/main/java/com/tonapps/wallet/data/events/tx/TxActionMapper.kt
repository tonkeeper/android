...
internal class TxActionMapper(
    ...
    private fun Price.toAmount(): TxActionBody.Value {
        val currency = currency(this)
        val amount = Coins.ofNano(value, decimals)
        return TxActionBody.Value(amount, currency)
    }
    ...
    private fun withdrawStakeRequest(address: BlockchainAddress, action: WithdrawStakeRequestAction): TxActionBody {
        ...
        val ingoingAmount = action.stakeMeta?.toAmount()
        builder.setRecipient(recipient)
        ingoingAmount?.let(builder::setOutgoingAmount)
        return builder.build()
    }
    ...
    private fun depositStake(address: BlockchainAddress, action: DepositStakeAction): TxActionBody {
        ...
        val builder = TxActionBody.Builder(ActionType.DepositStake)
        action.stakeMeta?.toAmount()
            ?.let { builder.setOutgoingAmount(it) }
            ?: builder.setOutgoingAmount(Coins.of(action.amount))

...
data class StakedEntity(
    ...
    companion object {
        suspend fun create(
            ...
            includeEmptyBalances: Boolean = false,
        ): List<StakedEntity> {
            val fiatRates = ratesRepository.getTONRates(wallet.network, currency)
            val list = mutableListOf<StakedEntity>()
            val pools = if (includeEmptyBalances) getAllPools(staking) else getActivePools(staking, tokens)
            for (pool in pools) {
                if (pool.implementation == StakingPool.Implementation.LiquidTF) {
                    ...
                    val token = tokens.find { it.address.equalsAddress(liquidJettonMaster) }
                    if (token == null) {
                        if (!includeEmptyBalances) continue
        ...
        private fun getAllPools(staking: StakingEntity): List<PoolEntity> {
            return staking.pools.flatMap { it.pools }
        }

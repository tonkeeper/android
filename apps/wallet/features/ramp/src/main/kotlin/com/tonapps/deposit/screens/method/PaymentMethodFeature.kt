...
sealed interface PaymentMethodEvent {
    ...
    data class AutoNavigateCrypto(
        val resolvedAsset: RampAsset,
        val targetCurrency: WalletCurrency
    ) : PaymentMethodEvent

    data class AutoNavigateCash(
        val resolvedAsset: RampAsset,
...
sealed interface PaymentMethodState : MviState {
    ...
    data class Data(
        ...
        val showCashSection: Boolean
            get() = paymentMethods.isNotEmpty()
                    && (sectionFilter == PaymentMethodSectionFilter.All
                    || sectionFilter == PaymentMethodSectionFilter.CashOnly)
        val showCryptoSection: Boolean
            get() = cryptoAssets.isNotEmpty()
                    && (sectionFilter == PaymentMethodSectionFilter.All
                    || sectionFilter == PaymentMethodSectionFilter.CryptoOnly)
        val showStablecoinSection: Boolean
...
class PaymentMethodFeature(
    ...
    override suspend fun executeAction(action: PaymentMethodAction) {
        when (action) {
            ...
            is PaymentMethodAction.CheckP2PMethod -> {
                ...
                if (shouldValidated) {
                    val merchant = exchangeRepository.getMerchants()
                ...
                try {
                    ...
                    relay.emit(PaymentMethodEvent.ShowP2PError)
                    L.e(e)
                }
            }
    ...
    private suspend fun loadData() {
        ...
        try {
            ...
            val selectedCurrency = preferredCurrency
    ...
    private suspend fun emitDeeplinkNavigation(
    ...
    private suspend fun createP2PSession(address: String): String {
        ...
        val fiatCurrency = state?.selectedCurrency?.code ?: error("Currency must be specified")

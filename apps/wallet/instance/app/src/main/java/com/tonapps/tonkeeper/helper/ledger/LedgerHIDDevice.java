package com.tonapps.tonkeeper.helper.ledger;
...
// Source: https://github.com/LedgerHQ/ledger-live/blob/develop/libs/ledgerjs/packages/react-native-hid/android/src/main/java/com/ledgerwallet/hid/HIDDevice.java
// TODO: Refactor this pizdec
public class LedgerHIDDevice {
    ...
    public LedgerHIDDevice(UsbManager manager, UsbDevice device) {
    ...
    public void exchange(final byte[] commandSource, final Callback callback) throws Exception {
        Runnable exchange = new Runnable() {
            public void run() {
                try {
                    ...
                    request.close();
                    callback.accept(toHex(responseData));
                } catch (Exception e) {
                    Log.d("SHIDDevice", "Error: " + e.getMessage(), e);
                    callback.reject(e);
                }
            }
        };

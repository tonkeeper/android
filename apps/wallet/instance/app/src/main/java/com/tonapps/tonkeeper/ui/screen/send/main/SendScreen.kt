    ...
    private fun setToken(token: TokenEntity) {
        ...
        collectFlow(viewModel.tronAvailableFlow.take(1)) { isTronAvailable ->
            if (isTronAvailable && (token.isUsdt || token.isUsdtTrc20)) {
                ...
                reviewSubtitleView.text = getString(Localization.jetton_transfer, CurrencyFormatter.displaySymbol(token.symbol))
            ...
            when (fee) {
                is SendFee.TokenFee -> {
                    ...
                    feeMethodSelector.addItem(
                        ...
                        title = CurrencyFormatter.displaySymbol(fee.amount.token.symbol),
                        subtitle = subtitle,

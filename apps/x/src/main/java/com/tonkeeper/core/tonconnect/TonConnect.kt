...
class TonConnect(private val context: Context) {
    ...
    private fun onEvent(event: TCEvent) {
        scope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                val seqno = TransactionHelper.getSeqno(accountId, wallet.testnet)
                ...
                val response = Tonapi.emulation.get(wallet.testnet).emulateMessageToWallet(request)
    ...
    private fun sendToBlockchain(boc: String, testnet: Boolean) {
        val request = SendBlockchainMessageRequest(boc)
        blockchainApi.get(testnet).sendBlockchainMessage(request)
    }

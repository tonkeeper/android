...
class ConfirmScreenFeature: UiFeature<ConfirmScreenState, ConfirmScreenEffect>(ConfirmScreenState()) {
    ...
    private suspend fun transferMessage(tx: TransactionData) = withContext(Dispatchers.IO) {
        try {
            ...
            val account = accountRepository.getFromCloud(wallet.accountId, wallet.testnet)?.data ?: throw Exception("failed to get account")
            ...
            if (tx.isTon) {
                ...
                TransactionHelper.sendTon(wallet, tx.address!!, amount, tx.comment, tx.max, tx.bounce, wallet.testnet)
            } else {
                TransactionHelper.sendJetton(wallet, tx.jetton!!, tx.address!!, amount, tx.comment, tx.bounce, wallet.testnet)
            }
    ...
    private suspend fun calculateFeeJetton(
        ...
        val emulate = TransactionHelper.emulateJetton(wallet, jetton, to, value, comment, bounce, wallet.testnet)
        if (emulate == null) {
            updateUiState {
                it.copy(
                    feeValue = 0,
            ...
            val feeInCurrency = from(jetton.address, wallet.accountId, wallet.testnet)
                .value(emulate.fee)
                .to(currency)
    ...
    private suspend fun calculateFeeTon(
        ...
        val emulate = TransactionHelper.emulateTon(wallet, to, value, comment, max, bounce, wallet.testnet)
        if (emulate == null) {
            updateUiState {
                it.copy(
                    feeValue = 0,
